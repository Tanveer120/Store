...
selectedBg = bg[0];
backgroundPreview.style.backgroundImage = "url('https://images.pexels.com/photos/255379/pexels-photo-255379.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1')";
...
selectedModel = modelsArray[2];
loadGlbModelUrl(modelsArray[2].url);
...
function updateTextDecals(Mapid, parentId) {
  ...
  // Get the current color from the color picker instead of hardcoding it
  let color = document.getElementById("TextColor_" + parentId).value || "#000000";
  let fontFamily = document.getElementById("ChooseFontFamily_" + parentId).value;
  let size = parseFloat(document.getElementById("ChooseFontSize_" + parentId).value);
  ...
  let properties = {
    ...
    text,
    fontSize: size,
    color, // update the property here
    fontFamily: texture.fontFamily,
    scale: sizeP,
    position: position,
    orientation: rot,
...
function attachColorChangeEvent() {
  // Attach change event to all color picker inputs for text decals
  document.querySelectorAll('.textColor').forEach((input) => {
    input.addEventListener('change', (event) => {
      const parentId = event.target.getAttribute('data-id');
      let mapidElement = document.getElementById("TextB" + parentId);
      if (mapidElement) {
        let mapid = mapidElement.value;
...
// Call the function after your UI is loaded
...
// Utility functions to set and get cookies with proper encoding
...
// Make sure the Cloudinary widget script is loaded before this runs.
if (!window.cloudinary) {
  console.error("Cloudinary widget not loaded. Please include the Cloudinary widget script in your HTML.");
}

// Replace with your Cloudinary cloud name and your unsigned upload preset name.
const cloudName = "dl9k1eipe"; // e.g., "mycloudname"
const uploadPreset = "Customizer"; // e.g., "unsigned_preset_name"
...
const myWidget = cloudinary.createUploadWidget(
  ...
  (error, result) => {
    if (!error && result && result.event === "success") {
      ...
      // Save the Cloudinary URL to selectedDecalImg
      selectedDecalImg = result.info.secure_url;
      
      // Optionally update an image preview element if present
      const preview = document.getElementById("uploadedImagePreview");
      if (preview) {
        preview.src = selectedDecalImg;
      }
      
      // Use the uploaded image as the decal by calling uploadDesign()
      // This function loads the image, creates a texture, and places the decal.
      uploadDesign(selectedDecalImg);
    } else if (error) {
...
// // Add an event listener to your upload button to open the widget
...
//   });
// } else {
//   console.warn("Upload button with id 'upload_widget_button' not found.");
// }

// --- Your Existing Code ---
// ... rest of your script.js code (including your 3D scene, customizer logic, etc.)
...
// Create a Buy Now button and append it to the settings panel UI (or any container)
function addBuyNowButton() {
  const buyNowButton = document.createElement("button");
  buyNowButton.innerText = "Buy Now";
  buyNowButton.classList.add("btn", "btn-primary");
  buyNowButton.style.marginTop = "20px";

  let decalImage = decals.filter((value) => value.type === "image");
  
  buyNowButton.addEventListener("click", () => {
    // Gather custom order details from your customizer state.
    // Ensure these variables are updated by your script.js logic:
    // selectedColor, selectedModel, intersection.point, mouseHelper, selectedDecalImg, meshParam, etc.
    const textContent = (mouseHelper.name === "Text" &&
                         mouseHelper.properties &&
                         mouseHelper.properties.text)
                         ? mouseHelper.properties.text
                         : "";
    const textPosition = (mouseHelper.name === "Text")
                         ? {
    ...
    const orderData = {
      ...
      tshirtColor: selectedColor.value, // e.g., "222f3e"
      model: {
        id: selectedModel.id,
      ...
      decal: {
        imageUrl: selectedDecalImg || "", // URL for decal image (backend will handle upload if needed)
        position: {
          x: intersection.point.x,
          y: intersection.point.y,
          z: intersection.point.z
        ...
        rotation: {
          x: mouseHelper.rotation.x,
          y: mouseHelper.rotation.y,
          z: mouseHelper.rotation.z
        },
        scale: {
          x: (mouseHelper.properties && mouseHelper.properties.scale && mouseHelper.properties.scale.x) || meshParam.scale,
          y: (mouseHelper.properties && mouseHelper.properties.scale && mouseHelper.properties.scale.y) || meshParam.scale,
          z: 1
        },
        textColor: (mouseHelper.properties && mouseHelper.properties.color) || "#000000",
    ...
    // Store the custom order data as a JSON string in a cookie named "customOrder"
    setCookie("customOrder", JSON.stringify(orderData), 1);
    
    // Redirect to your PlaceCustomOrder page on localhost:5173
    window.location.href = "http://localhost:5173/custom-order";
  });

  // Append the Buy Now button to the settings panel container if it exists,
  // otherwise, append it to the body.
  if (settingLayoutUI) {
    settingLayoutUI.appendChild(buyNowButton);
  } else {
    document.body.appendChild(buyNowButton);
  }
}

// Call your UI loader then add the Buy Now button
loadSettingsUI();
addBuyNowButton();
